// Lógica específica para la tabla aproximados (usa id_correria como clave única)
let tableColumns = [];
let currentData = [];
...
function exportToExcel() {
    ...
    try {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, 'Aproximados');
        
        // Generar nombre de archivo con fecha
        const fecha = new Date().toISOString().split('T')[0];
        const filename = `aproximados_${fecha}.xlsx`;
...
async function limpiarTabla() {
    const confirmText = prompt(
        'ADVERTENCIA: Esto eliminará TODOS los registros de la tabla aproximados.\n\n' +
